      ...
      */
        ...
        fedEx ) /* the FederatesExporter function object */
      {
        ...
        var dateString = ( year + "-" + ( ( month < 10 ) ? "0" : "" ) + month +
          "-" + ( ( day < 10 ) ? "0" : "" ) + day );
        var xmlModel; // model from which to generate XML
        ...
        fedEx.fileGenerators.push( function( artifact, callback )
        {
          for ( federId in fedEx.federateInfos )
          {
            ...
            // [adlaws] reminder to self: remove this spurious logging once you're done!
            //          anyone else: please feel free to delete this, and my apologies [adlaws]
            console.log(fedEx.interactionRoots);
            ...
            fedEx.interactionRoots.forEach( function( interactionRoot )
            {
              ...
              xmlModel.interactionsXml.push( interactionTraverserXml( feder, interactionRoot, "    " ) );
            } );

            fedEx.objectRoots.forEach( function( objectRoot )
            {
              //objectTraverserCheck(feder, objectRoot);
              xmlModel.objectsXml.push( objectTraverserXml( feder, objectRoot, "    " ) );
            } );

            ['fom','som'].forEach( function( xmlKind )
              {
                var currentTemplate = TEMPLATES[ 'fedfile-'+xmlKind+'.xml.ejs' ];
                var currentXML = ejs.render( currentTemplate, xmlModel );
                var currentPath = directory + xmlKind + '.xml';
                artifact.addFile( currentPath, currentXML, function( err ) { if ( err ) { callback( err ); return; } } );
               }
            );

            // add fom XML files to artifact
            fullPath = directory + feder.name + '.xml';
            xmlCode = ejs.render( template, xmlModel );
            console.log( 'calling addFile for: ' + fullPath );
            artifact.addFile( fullPath, xmlCode,
              ( remaining ?
                // there are more
                function( err ) { if ( err ) { callback( err ); return; } } :
                // last one
                function( err ) { if ( err ) { callback( err ); return; } else { callback(); } }
              )
            );
        ...
        callback ) /* callback function */
      {
          ...
          if ( !errorRaised )
          {
            self.blobClient.saveAllArtifacts( function( err, hashes )
            {
              ...
              for ( idx = 0; idx < hashes.length; idx++ )
              {
                ...
                buildURL =
                  "'http://c2w-cdi.isis.vanderbilt.edu:8080/job/c2w-pull/buildW ithParameters?GME_ARTIFACT_ID=" +
                  hashes[ idx ] + "'";
        ...
        */
          ...
          err ) /* (string) an error string or null (?) */
        {
          ...
          artifact =
            self.blobClient.createArtifact( self.projectName.trim().replace( /\s+/g, '_' ) + '_generated' );
          fomGenerator( self );
